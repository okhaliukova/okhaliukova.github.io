...
We develop a Bayesian nonparametric approach to estimate methane emission distributions, which are often heavy-tailed, using a log-histospline (LHSpline) framework. The method models the log-density via smoothing splines, accurately capturing both the bulk and tail behavior, particularly for distributions with exponential tails. We extend LHSpline to a fully Bayesian framework, enabling uncertainty quantification through posterior distributions and credible intervals. Because site-level emissions are typically computed as sums of multiple sources, we also develop methods for directly estimating extreme quantiles of the aggregate distribution without relying on Monte Carlo simulation. The approach is broadly applicable beyond environmental science to domains such as finance, insurance, and risk analysis, where sums of heavy-tailed variables are common. We demonstrate its performance using high-frequency methane monitoring data from an oil and gas production facility in the Appalachian Basin.
